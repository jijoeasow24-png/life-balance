...
function scheduleSync(state) {
  ...
  syncTimer=setTimeout(()=>{ fetch('/api/data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(state)}).catch(()=>{}); },1500);
}
function blockedMins(blocks,domain,date) {
  return safeArr(blocks,[]).filter(b=>b.domain===domain&&occursOn(b,date)).reduce((acc,b)=>{ const [sh,sm]=b.start.split(':').map(Number),[eh,em]=b.end.split(':').map(Number); return acc+(eh*60+em-sh*60-sm); },0);
}
function layoutBlocks(blocks) {
  ...
  sorted.forEach(block=>{
    const bStart=toMins(block.start),bEnd=toMins(block.end);
    let placed=false;
    for(let c=0;c<columns.length;c++){
      if(toMins(columns[c][columns[c].length-1].end)<=bStart){ columns[c].push(block); result.push({block,col:c}); placed=true; break; }
    }
    if(!placed){ columns.push([block]); result.push({block,col:columns.length-1}); }
  });
  result.forEach(item=>{
    const bStart=toMins(item.block.start),bEnd=toMins(item.block.end);
    let maxCol=item.col;
  ...
  return (
    <div style={{background:'var(--surface2)',borderRadius:10,padding:12,marginBottom:8}}>
      <div style={{display:'flex',gap:8,marginBottom:8}}>
        <input style={{flex:1}} value={f.title} onChange={e=>setF(x=>({...x,title:e.target.value}))}/>
      </div>
      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
        <select value={f.domain} onChange={e=>setF(x=>({...x,domain:e.target.value}))}>
          {Object.entries(DOMAINS).map(([k,d])=><option key={k} value={k}>{d.label}</option>)}
    ...
    <div style={{position:'relative'}}>
      <div style={{position:'relative',height:HOURS*HOUR_H}}>
        {Array.from({length:HOURS+1},(_,i)=>i+START_H).map(h=>{
          ...
          return (
            <div key={h} style={{position:'absolute',top,left:0,right:0,display:'flex',gap:10}}>
        ...
        {laid.map(({block,col,totalCols})=>{
          ...
          const top=(startM-START_H*60)/60*HOUR_H;
          const height=Math.max((endM-startM)/60*HOUR_H-4,20);
          const L=62,colW=`calc((100% - ${L}px) / ${totalCols})`;
          const left=`calc(${L}px + ${col} * (100% - ${L}px) / ${totalCols})`;
          const d=DOMAINS[block.domain];
          if(editId===block.id) return (
            <div key={block.id} style={{position:'absolute',top,left,width:colW,zIndex:20,paddingRight:4}}>
              <BlockEditForm block={block} onSave={onSaveEdit} onCancel={()=>setEditId(null)}/>
            </div>
          );
          return (
