...
// Does a block recur on a given Date?
...
function sanitize(data) {
  if (!data || typeof data !== 'object') return { tasks: [], blocks: [], week: DEFAULT_WEEK, streak: { count: 1, lastDate: todayKey() } };
  return {
    tasks:  safeArr(data.tasks,  []),
    blocks: safeArr(data.blocks, []),
...
function lsLoad() {
  try { const r = localStorage.getItem('lb_app'); return r ? sanitize(JSON.parse(r)) : null; } catch { return null; }
}
...
function Schedule({ blocks, onChange }) {
  ...
  const [offset, setOffset] = useState(0); // days/weeks/months offset from today
  ...
  // Compute the "anchor" date based on view + offset
  function anchorDate() {
  ...
  function viewLabel() {
    ...
    if (view==='day') {
      ...
      return isToday ? 'Today' : anchor.toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'short'});
    }
    if (view==='week') {
  ...
  const add = () => {
    ...
    onChange([...sb, {...form, id:maxId+1, date:dateStr}]);
    setForm(f=>({...f, title:''}));
  };
  ...
  return (
    <div>
      {/* Add form */}
      <div style={{...s.s2, marginBottom:20}}>
      ...
      <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:16}}>
        <button onClick={()=>setOffset(o=>o-1)} style={{background:'none', border:'0.5px solid var(--border)', borderRadius:6, padding:'4px 10px', cursor:'pointer', color:'var(--text)'}}>‹</button>
      ...
      {/* Day view */}
      {view==='day' && (
        <DayTimeline date={dates[0]} blocks={sb} onDelete={id=>onChange(sb.filter(x=>x.id!==id))}/>
      )}
            ...
            return (
              <div key={date.toISOString()} style={{marginBottom:12}}>
                ...
                {dayBlocks.length===0
                  ...
                  : dayBlocks.map(b=><BlockPill key={b.id} block={b} onDelete={()=>onChange(sb.filter(x=>x.id!==b.id))}/>)
                }
              </div>
            );
          })}
      ...
      {view==='month' && (
        <div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4}}>
          {['M','T','W','T','F','S','S'].map((d,i)=>(
            <div key={i} style={{fontSize:11, color:'var(--text2)', textAlign:'center', padding:'4px 0'}}>{d}</div>
          ))}
          {/* leading empty cells */}
            ...
            return (
              <div key={date.toISOString()} style={{border:`0.5px solid ${isToday?'var(--text)':'var(--border)'}`, borderRadius:8, padding:'4px 6px', minHeight:56, background:isToday?'var(--surface2)':'none'}}>
                ...
                {dayBlocks.slice(0,2).map(b=>{
                  ...
                  return <div key={b.id} style={{fontSize:9, background:d.light, color:d.dark, borderRadius:3, padding:'1px 4px', marginBottom:2, overflow:'hidden', whiteSpace:'nowrap', textOverflow:'ellipsis'}}>{b.title}</div>;
                })}
                {dayBlocks.length>2&&<div style={{fontSize:9, color:'var(--text3)'}}>+{dayBlocks.length-2} more</div>}
              </div>
            );
          })}
        </div>
...
function DayTimeline({ date, blocks, onDelete }) {
  const now = new Date();
  ...
  return (
    <div>
      {Array.from({length:18},(_,i)=>i+5).map(h=>{
        const label = fmtTime(`${String(h).padStart(2,'0')}:00`).replace(':00','');
        ...
        return (
          <div key={h} style={{display:'flex', gap:10, minHeight:40, marginBottom:2}}>
            ...
            <div style={{flex:1, borderLeft:'0.5px solid var(--border)', paddingLeft:10, paddingTop:3}}>
              ...
              {here.map(b=><BlockPill key={b.id} block={b} onDelete={()=>onDelete(b.id)} showTime/>)}
            </div>
          </div>
        );
      })}
...
function BlockPill({ block, onDelete, showTime }) {
  const d = DOMAINS[block.domain];
  const recurLabel = RECUR_OPTIONS.find(o=>o.value===block.recur)?.label;
  return (
    <div style={{padding:'5px 9px', borderRadius:8, marginBottom:4, background:d.light, borderLeft:`3px solid ${d.color}`, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
      <div>
      ...
      <button onClick={onDelete} style={{background:'none', border:'none', color:d.dark, fontSize:16, opacity:0.5, cursor:'pointer'}}>×</button>
    </div>
  );
}
...
function Tasks({ tasks, onChange }) {
  ...
  const add = () => {
    if (!form.title.trim()) return;
    onChange([...t, {...form, id:maxId+1, done:false}]);
    setForm(f=>({...f,title:''}));
  };
      ...
      {visible.map(task=>{
        ...
        return (
          <div key={task.id} style={{display:'flex', alignItems:'center', gap:10, padding:'9px 0', borderBottom:'0.5px solid var(--border)'}}>
            ...
            <span style={s.pill(d.light,d.dark)}>{d.label}</span>
            <span style={{...s.pill(p.bg,p.txt), marginLeft:4}}>{task.pri}</span>
      ...
      <div style={{maxWidth:680, margin:'0 auto', padding:'0 16px 80px'}}>
        ...
        <div style={{display:'flex', borderBottom:'0.5px solid var(--border)', marginBottom:24, overflowX:'auto'}}>
          {['dashboard','schedule','tasks','progress'].map(t=>(
            <button key={t} style={s.nav(tab===t)} onClick={()=>setTab(t)}>
